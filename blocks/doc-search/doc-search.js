...
import { domEl } from '../../scripts/utils/dom-helpers.js';
...
function identifySource(a) {
  const fallback = '/blogpages-index.json';
...
function buildResult(match, terms, isHomepage) {
  ...
  const result = domEl('a', { href: match.path });
  ...
  const title = domEl('p', {}, truncate(match.title));
  const desc = domEl('p', {}, truncate(match.description));
  ...
  if (isHomepage) {
    ...
    const li = domEl('li', { class: 'doc-search-result' });
    li.append(result);
    return li;
  }
  ...
  const cardImage = domEl('div', { class: 'article-card-image' }, image);
  const cardBody = domEl('div', { class: 'article-card-body' });
  const span = domEl('span', { class: 'link-highlight-colorful-effect-2' }, title.innerHTML);
  const h3 = domEl('h3', {}, span);
  const p = domEl('p', {}, desc);
...
function findDoc(query, docs = [], findMultiple = false) {
  if (docs.length) {
    ...
    const filterOut = ['and', 'but', 'can', 'for', 'how', 'the', 'with', 'use', 'what', 'shape'];
    const terms = [query];
    terms.push(...query.toLowerCase().split(' ').map((e) => {
      e = e.trim();
      e = e.replaceAll(/[.!?]/g, '');
      if (e.endsWith('s')) e = e.slice(0, -1);
      return e;
    }).filter((e) => e.length > 2 && !filterOut.includes(e) && e !== query.toLowerCase()));
    ...
    indexDocs.forEach((doc) => {
      ...
      terms.forEach((term) => {
        if (doMatch(doc.title, term)) {
          found.rank = 1;
          found.count += 1;
        }
        if (doMatch(doc.heading, term)) {
          if (!found.rank || found.rank > 2) found.rank = 2;
          found.count += 1;
        }
        if (doMatch(doc.description, term)) {
          if (!found.rank || found.rank > 3) found.rank = 3;
          found.count += 1;
        }
        if (doMatch(doc.blurb, term)) {
          found.rank ||= 4;
          found.count += 1;
        }
      });
      if (found.count) matches.push(found);
    });
    ...
    return { terms, match: findMultiple ? results : results.slice(0, 3) };
  }
...
function type(placeholder, input, results, cb, isHomepage) {
  ...
  manageInterval(input, 40, () => {
    ...
    if (i >= placeholder.length) { // check if full placeholder is displayed
      ...
      searchQuery(placeholder, window.docs, results, isHomepage);
    }
  });
}
...
export default async function decorate(block) {
  ...
  const search = domEl('input', { type: 'search', 'aria-label': 'Search the documentation' });
  const clear = domEl('button', { type: 'reset' }, '✕');
